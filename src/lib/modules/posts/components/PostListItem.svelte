<script lang="ts">
	...
	let { post, showSummary = true, seriesBadge, active = false }: Props = $props();
</script>

<div
	class="group relative block w-full text-left transition-all duration-300 {active
		? 'bg-surface-elevated/30'
		: 'hover:bg-surface-elevated/10'}"
>
	{#if !active}
		<a href="/posts/{post.slug}" class="absolute inset-0 z-20">
			<span class="sr-only">Ver artigo {post.title}</span>
		</a>
	{/if}

	<div class="flex items-start gap-4 px-4 py-5 md:gap-5">
		<!-- Active Border Line -->
		{#if active}
			<div
				class="bg-primary absolute top-0 bottom-0 left-0 w-[3px]"
				aria-hidden="true"
			></div>
		{/if}
